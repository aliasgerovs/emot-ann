...
def build_comparison_excel(df1, df2):
    ...
    for r, clip in enumerate(all_clips):
        ...
        if missing_in:
            ...
            ws.merge_cells(start_row=row_excel, start_column=c1_start,
                           end_row=row_excel,   end_column=last_col)
            mc = ws.cell(row_excel, c1_start, f"MISSING (not coded by {missing_in})")
            mc.font      = Font(bold=True, italic=True, name="Arial", size=10, color="7B0099")
...
def export_final_csv(xlsx_bytes):
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        # A MISSING row has a MergedCell at final_start_col
        check_cell = ws.cell(row[0].row, final_start_col)
        if isinstance(check_cell, MergedCell):
            missing_rows.append(clip_num)
            continue
...
with gr.Blocks(title="Reliability Comparison Tool") as demo:
    gr.HTML("""
    ...
    def handle_compare(csv1_file, csv2_file):
        ...
        import tempfile, os
        fname = f"{summary['participant']}_{summary['task']}_comparison.xlsx"
        tmp   = tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx", prefix="comparison_")
        ...
        return msg, gr.update(value=tmp.name, visible=True, label=f"⬇️ {fname}")
    ...
    def handle_export(excel_file):
        ...
        import tempfile
        # Derive filename from Excel file name
        base = os.path.splitext(os.path.basename(excel_file.name))[0]
        base = base.replace("_comparison", "")
        fname = f"{base}_FINAL.csv"

        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".csv", prefix="final_")
